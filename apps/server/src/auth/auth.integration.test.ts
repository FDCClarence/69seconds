...
describeWithDatabase('authentication HTTP integration', () => {
  ...
  it('registers, normalizes credentials, stores an Argon2id hash, and returns a safe user', async () => {
    const response = await request(app)
      .post('/api/auth/register')
      .send({ username: '  Cart_Goblin ', email: '  PLAYER@Example.COM ', password: 'correct-horse-battery' })
      .expect(201);

    expect(response.body).toMatchObject({ user: { username: 'cart_goblin', email: 'player@example.com' } });
    ...
    expect(stored[0]?.username).toBe('cart_goblin');
  ...
  it('rejects duplicate normalized emails and usernames with distinct codes', async () => {
    await request(app).post('/api/auth/register')
      .send({ username: 'cart_goblin', email: 'player@example.com', password: 'correct-horse-battery' }).expect(201);
    const duplicateEmail = await request(app).post('/api/auth/register')
      .send({ username: 'other_goblin', email: ' PLAYER@example.com ', password: 'another-long-password' }).expect(409);
    expect(duplicateEmail.body.error.code).toBe('EMAIL_ALREADY_REGISTERED');
    const duplicateUsername = await request(app).post('/api/auth/register')
      .send({ username: ' Cart_Goblin ', email: 'other@example.com', password: 'another-long-password' }).expect(409);
    expect(duplicateUsername.body.error.code).toBe('USERNAME_ALREADY_TAKEN');
  });
  ...
  it('rejects invalid credentials without revealing which credential failed', async () => {
    ...
    const wrongPassword = await request(app).post('/api/auth/login')
      .send({ identifier: 'player@example.com', password: 'definitely-not-right' }).expect(401);
    const missingUser = await request(app).post('/api/auth/login')
      .send({ identifier: 'missing@example.com', password: 'definitely-not-right' }).expect(401);
    expect(wrongPassword.body.error.code).toBe('INVALID_CREDENTIALS');
    expect(missingUser.body.error).toEqual(wrongPassword.body.error);
  });

  it('logs in with either credential and replaces the presented session', async () => {
    ...
    expect(current.body.user).toMatchObject({ username: 'cart_goblin', email: 'player@example.com' });
    // The username login left a session of its own; only the agent's original session was replaced.
  ...
  it('validates request bodies and rate-limits credential endpoints', async () => {
    ...
    await request(limitedApp).post('/api/auth/login')
      .send({ identifier: 'missing@example.com', password: 'bad' }).expect(401);
    const limited = await request(limitedApp).post('/api/auth/login')
      .send({ identifier: 'missing@example.com', password: 'bad' }).expect(429);
